...
public class TripDao implements TripDaoInterface {
    ...
    public List<Trip> getAllUserTrip(User user) {
        ...
        Query query = entityManager.createQuery("select t from Trip t where t.user.id=" + user.getId() + " and t.endDate>'" + date + "'", Trip.class);
        List<Trip> result = query.getResultList();
    ...
    public List<Trip> getTripUserTravel() {
        ...
        result.forEach(trip -> {
            ...
            try {
                if (DateService.stringToDate(trip.getEndDate()).before(d)) {
                    toRemove.add(trip);
                }
